export default class KanbanAPI {
    ...
    static updateItem(itemId, newProps) {
        ...
        if(newProps.columnId !== undefined && newProps.position !== undefined) {
            const tragetColumn = data.find(column => column.id === newProps.columnId);
            if(!tragetColumn) throw new Error("Target not found");

            // Delete item from current column
            currentColumn.items.splice(currentColumn.items.indexOf(item), 1);

            // Move item into new column and position
            tragetColumn.items.splice(newProps.position, 0, item);
        }
